...
dist: $(DIST_TARGETS) | $(WINE_OUT) $(filter $(MAKECMDGOALS),wine64 wine32 wine) $(DST_DIR)
	WINEPREFIX=$(abspath $(DIST_PREFIX)) $(WINE_OUT_BIN) wineboot && \
		...
		ln -sf $(FONTLINKPATH)/LiberationSans-Regular.ttf $(abspath $(DIST_PREFIX))/drive_c/windows/Fonts/arial.ttf && \
		ln -sf $(FONTLINKPATH)/LiberationSans-Bold.ttf $(abspath $(DIST_PREFIX))/drive_c/windows/Fonts/arialbd.ttf && \
		ln -sf $(FONTLINKPATH)/LiberationSerif-Regular.ttf $(abspath $(DIST_PREFIX))/drive_c/windows/Fonts/times.ttf && \
		ln -sf $(FONTLINKPATH)/LiberationMono-Regular.ttf $(abspath $(DIST_PREFIX))/drive_c/windows/Fonts/cour.ttf
...
FFMPEG_OUT64 = $(abspath $(TOOLS_DIR64))/lib/libavutil.56.dylib
FFMPEG_OUT32 = $(abspath $(TOOLS_DIR32))/lib/libavutil.56.dylib

$(FFMPEG_OUT64): SHELL = $(CONTAINER_SHELL64)
$(FFMPEG_OUT64): $(FFMPEG_CONFIGURE_FILES64)
	+$(MAKE) -C $(FFMPEG_OBJ64)
...
ffmpeg32: $(FFMPEG_OUT32)
ffmpeg64: $(FFMPEG_OUT64)

$(FFMPEG_OUT32): SHELL = $(CONTAINER_SHELL32)
$(FFMPEG_OUT32): $(FFMPEG_CONFIGURE_FILES32)
	+$(MAKE) -C $(FFMPEG_OBJ32)
...
FAUDIO_CMAKE_FLAGS = -GNinja -DCMAKE_BUILD_TYPE=Release -DFORCE_ENABLE_DEBUGCONFIGURATION=ON -DLOG_ASSERTIONS=ON -DCMAKE_INSTALL_LIBDIR="lib" -DXNASONG=OFF
ifeq ($(WITH_FFMPEG),1)
...
FAUDIO_CONFIGURE_FILES32 := $(FAUDIO_OBJ32)/build.ninja
FAUDIO_CONFIGURE_FILES64 := $(FAUDIO_OBJ64)/build.ninja
FAUDIO_OBJ_FILES32 := $(abspath $(TOOLS_DIR32))/lib/libFAudio.$(LIB_SUFFIX)
FAUDIO_OBJ_FILES64 := $(abspath $(TOOLS_DIR64))/lib/libFAudio.$(LIB_SUFFIX)
...
faudio32: $(FAUDIO_OBJ_FILES32)
faudio64: $(FAUDIO_OBJ_FILES64)
...
# ifeq ($(OSX),1)
# 	install_name_tool -change "$$(cd "$(TOOLS_DIR32)" && pwd -P)/lib/libSDL2-2.0.0.dylib" "@rpath/libSDL2.dylib" $(DST_DIR)/lib/libFAudio.$(LIB_SUFFIX)
# endif
	[ x"$(STRIP)" = x ] || $(STRIP) $(DST_DIR)/lib/libFAudio.$(LIB_SUFFIX)


$(FAUDIO_OBJ_FILES64): SHELL = $(CONTAINER_SHELL64)
$(FAUDIO_OBJ_FILES64): $(FAUDIO_CONFIGURE_FILES64)
	ninja -C $(FAUDIO_OBJ64) install
	mkdir -p $(DST_DIR)/lib64
	cp -L $(TOOLS_DIR64)/lib/libFAudio* $(DST_DIR)/lib64/
# ifeq ($(OSX),1)
# 	install_name_tool -change "$$(cd "$(TOOLS_DIR64)" && pwd -P)/lib/libSDL2-2.0.0.dylib" "@rpath/libSDL2.dylib" $(DST_DIR)/lib64/libFAudio.$(LIB_SUFFIX)
# endif
...
$(LSTEAMCLIENT_CONFIGURE_FILES64): $(LSTEAMCLIENT64) $(MAKEFILE_DEP) $(FAUDIO_OBJ_FILES64) | $(LSTEAMCLIENT_OBJ64) $(WINEMAKER)
	cd $(dir $@) && \
...
$(LSTEAMCLIENT_CONFIGURE_FILES32): $(LSTEAMCLIENT32) $(MAKEFILE_DEP) $(FAUDIO_OBJ_FILES32) | $(LSTEAMCLIENT_OBJ32) $(WINEMAKER)
	cd $(dir $@) && \
...
$(VRCLIENT32): $(VRCLIENT) $(MAKEFILE_DEP)
	...
	cd $(VRCLIENT32)/vrclient && \
		ln -sf ../../$(VRCLIENT)/vrclient_x64/*.{c,cpp,dat,h,spec} .
...
vrclient64: $(VRCLIENT_CONFIGURE_FILES64) | $(WINE_BUILDTOOLS64) $(filter $(MAKECMDGOALS),wine64 wine32 wine)
	...
	cd $(VRCLIENT_OBJ64) && \
		...
		mkdir -p ../$(DST_DIR)/lib64/wine/fakedlls && \
...
vrclient32: $(VRCLIENT_CONFIGURE_FILES32) | $(WINE_BUILDTOOLS32) $(filter $(MAKECMDGOALS),wine64 wine32 wine)
	...
	cd $(VRCLIENT_OBJ32) && \
		...
		mkdir -p ../$(DST_DIR)/lib/wine/fakedlls && \
...
ifeq ($(OSX),1)
include $(SRCDIR)/build/makefile_osx.mak
endif
